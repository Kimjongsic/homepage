...
<head>
    ...
    <style type="text/css">
        ...
        .post_content {
            border-top: 3px solid rgb(251, 188, 5);
        }
        .post_bar li a:hover {
            color: rgb(251, 188, 5);
        }
...
<?php
		...
		$hit = mysqli_fetch_array(mq("select * from sciboard where num ='".$bno."'"));
		$hit = $hit['hit'] + 1;
		$fet = mq("update sciboard set hit = '".$hit."' where num = '".$bno."'");
		$sql = mq("select * from sciboard where num='".$bno."'"); /* 받아온 idx값을 선택 */
...
<!-- 글 불러오기 -->
    ...
    <div class="listbtn_wrap">
        <div class="list_wrap">
            <a class="list_btn" href="sciboard.php">목록</a>
        </div>
    </div>
</body>
</html>